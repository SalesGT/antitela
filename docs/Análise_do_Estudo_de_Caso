...
A seguir, são detalhadas as diferentes condições de uso do aplicativo e seus impactos diretos no desenvolvimento da solução:

* **Ambiente:**
  * Quartos de dormir (frequentemente com a porta fechada e luz apagada), salas de aula/colégio, transporte público e mesas de estudo em casa.
* **Momento de utilização:**
  * Tarde (durante a execução de tarefas escolares), início da noite e, criticamente, no período pré-sono/madrugada (quando o uso compulsivo rouba horas de descanso).
* **Condições do usuário:**
  * Fadiga mental, irritabilidade, ansiedade gerada pela enxurrada de notificações, impulsividade e dificuldade de romper o ciclo de recompensas rápidas dos algoritmos de redes sociais.
* **Dispositivo:**
  * Smartphones pessoais do adolescente, em sua maioria aparelhos Android de entrada ou intermediários, com especificações modestas de hardware.
* **Conectividade:**
  * Conexão oscilante, alternando entre redes Wi-Fi residenciais/escolares e redes móveis (3G/4G) frequentemente com franquias de dados pré-pagas restritas.
* **Iluminação:**
  * Predominância de ambientes de baixa ou nula iluminação (uso deitado na cama no escuro) e luz artificial de salas de aula.
* **Nível de atenção:**
  * Altamente disperso, fragmentado e com baixa tolerância a interfaces lentas, formulários burocráticos ou menus complexos.
* **Situação de urgência:**
  * Momentos em que o jovem precisa contatar familiares com rapidez, atender a uma emergência de saúde ou receber comunicações cruciais do colégio, mesmo durante uma sessão de foco.
* **Outras condições específicas:**
  * Uso simultâneo de fones de ouvido e constante alternância rápida entre múltiplos aplicativos abertos em segundo plano.

### Implicações desses contextos para o desenvolvimento do aplicativo:
1. **Interface Escura Obrigatória (Dark Mode Nativo):** O contexto de baixa iluminação noturna exige paleta escura que amenize a emissão de luz azul e evite desconforto ocular.
2. **Arquitetura Offline-First:** Devido à instabilidade e limitações de conectividade, todas as funções essenciais (avatar, metas, diário e foco) devem operar sem internet.
3. **Usabilidade para Atenção Fragmentada:** Fluxos operacionais diretos, evitando telas intermediárias e carregamentos que incentivem a distração.
4. **Respeito a Urgências:** O aplicativo jamais deve assumir o papel de bloqueador autoritário do aparelho. Em caso de necessidade real, o usuário deve poder encerrar a sessão de foco sem travas mecânicas que impeçam chamadas essenciais.
...
### O que o aplicativo pretende oferecer e benefícios proporcionados:
...
* **Personalidade da Identidade:** Jovem, contemporânea, compreensiva e descontraída. O aplicativo se posiciona como um *aliado de guilda* ou um *parceiro de treino*, distanciando-se expressamente da postura de um fiscalizador ou coordenador disciplinar.
* **Tom da Interface:** Estética inspirada no universo gamer moderno (tema escuro, destaques em ciano elétrico e roxo suave). Uso de tipografia limpa e sem serifa, textos curtos em linguagem coloquial respeitosa, sem infantilizar o público jovem.
* **Tom da Experiência do Usuário (UX):** Não intrusiva, ágil e acolhedora. Erros ou desistências não são punidos com advertências humilhantes, mas sim com feedback sutil que incentiva a recomeçar.
...
| **Modo Foco com Silenciamento Nativo (DND)** | **Redução de Fricção e Estudo sem Distração:** Permite entrar em concentração quase instantaneamente, silenciando alertas no sistema operacional sem trancar a tela do dispositivo de forma autoritária. |
...
Em estrita conformidade com os requisitos do estudo de caso, as seguintes restrições balizam o projeto:

* **Quantidade de telas:**
  * Limite rigoroso de **até 4 telas principais** (Home/Avatar, Metas/Missões, Modo Foco e Diário de Humor), impedindo labirintos de navegação que gerem dispersão.
* **Número de interações:**
  * A ação central de iniciar uma sessão de foco deve ser executada em **no máximo 3 interações (toques)** a partir da abertura do app.
* **Dispositivos:**
  * Otimizado para smartphones de entrada e intermediários com capacidade de hardware modesta, com ênfase em aparelhos dotados de **2GB de memória RAM**.
* **Versão do sistema operacional:**
  * Compatível com **Android 8.0 (API nível 26) ou superior**, viabilizando o uso de canais de notificação e das permissões de monitoramento de uso (`UsageStatsManager`).
* **Tamanho do aplicativo:**
  * Arquitetura compacta e enxuta (baixo consumo de disco e APK reduzido), evitando a remoção do app em aparelhos com pouco espaço interno livre.
* **Privacidade:**
  * **Isolamento absoluto:** Os dados subjetivos do Diário de Humor (anotações pessoais, sentimentos e tags) são restritos ao aparelho do jovem. Nenhum desses registros é enviado para a nuvem ou exposto no relatório familiar.
* **Armazenamento:**
  * Persistência **100% local** empregando SQLite (`sqflite`) e chave-valor (`shared_preferences`), assegurando independência de servidores para funcionalidades de rotina.
* **Conectividade:**
  * Operação integral em modo offline. O envio de dados e background jobs (`workmanager`) é restrito estritamente a **conexões Wi-Fi ativas**, sem consumir a franquia móvel do usuário.
* **Navegação:**
  * Navegação linear, rasa e ergonômica (barra de navegação inferior visível), com transições fluidas e alvos de toque adequados.
* **Acessibilidade:**
  * Contraste tipográfico conforme padrões de legibilidade em fundo escuro, fontes legíveis e botões com área de toque mínima recomendada de 48x48 dp.
* **Ambiente de utilização:**
  * O aplicativo deve ser discreto e silencioso, sem disparar alertas sonoros involuntários em salas de aula ou no quarto em horário de repouso.
* **Outras condições específicas:**
  * **Não utilização de bloqueio forçado do sistema:** O aplicativo prioriza a autonomia moral. A desistência do foco é permitida a qualquer momento, aplicando apenas penalidades no jogo (gamificação), sem aprisionar o dispositivo do usuário.
